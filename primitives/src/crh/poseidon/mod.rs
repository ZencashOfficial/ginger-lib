...
mod test {
    ...
    #[test]
    fn test_poseidon_permutation_bn382_fr() {
        ...
        for i in 0..test_ins.len(){
            let mut state = test_ins[i].clone();
            BN382FrPoseidonHash::poseidon_perm(&mut state);
            assert_eq!(state, test_outs[i],"Output does not match BN382 Fr, test vector {:?}", i);
        }
        
    }

    #[test]
    fn test_poseidon_permutation_bn382_fq() {
        // test vectors are computed on random input using the sage script of 
        // [IAIK](https://extgit.iaik.tugraz.at/krypto/hadeshash), commit 7ecf9a7d4f37e777ea27e4c4d379443151270563. 
        let test_ins = vec![
            vec![BN382Fq::zero(); 3],
        ...
        for i in 0..test_ins.len(){
            let mut state = test_ins[i].clone();
            BN382FqPoseidonHash::poseidon_perm(&mut state);
            assert_eq!(state, test_outs[i],"Output does not match BN382 Fr, test vector {:?}", i);
        }
